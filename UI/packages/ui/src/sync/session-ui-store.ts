...
export const useSessionUIStore = create<SessionUIState>()((set, get) => ({
  ...
  openNewSessionDraft: (options) => {
    ...
    // A new session opened with no explicit project/directory context (the global "+", a
    // keyboard shortcut, the command palette, etc.) should follow whichever project is actually
    // active right now — not some other, unrelated project a session happened to be opened in at
    // some earlier point. useDirectoryStore's `currentDirectory` (persisted to localStorage as
    // `lastDirectory`) used to win here regardless of the active project, which is exactly why a
    // stale, unrelated project kept getting reused for every new draft. getActiveProject()
    // already returns null when nothing is genuinely active — that's the correct "blank" case,
    // not a reason to fall back to an arbitrary project or a stale remembered directory.
    ...
    const directory = (() => {
      ...
      if (activeProject) return normalizePath(activeProject.path ?? null)
      return null
    })()
