...
// Last path segment, slash-agnostic (Windows backslashes or POSIX). Plan paths are compared by
// basename because a stored path and the on-disk path can be rooted differently for the same file.
const basename = (value: string): string => normalize(value).split('/').pop() || value;
...
// Lists .md files directly in a directory (non-recursive). Used to reconcile the plans/ directory
// on disk against the stored projectPlanFiles list, so plan files written straight to disk (by the
// planner/plan-writer agents, or dropped in manually) show up in the Plans panel without having
// been created through the UI. Returns [] on any failure — a plans dir that doesn't exist yet is
// the normal first-run case, not an error.
const listMarkdownFilesIn = async (dir: string): Promise<Array<{ path: string; name: string; modifiedTime?: number }>> => {
  const collect = (entries: Array<{ name?: unknown; path?: unknown; isDirectory?: unknown; modifiedTime?: unknown }>) =>
    entries
      .filter((e) => !e.isDirectory && typeof e.name === 'string' && /\.md$/i.test(e.name as string))
      .map((e) => ({
        path: typeof e.path === 'string' && e.path ? (e.path as string) : joinPath(dir, e.name as string),
        name: e.name as string,
        modifiedTime: typeof e.modifiedTime === 'number' ? (e.modifiedTime as number) : undefined,
      }));

  const runtimeFiles = getRuntimeFilesAPI();
  if (runtimeFiles?.listDirectory) {
    try {
      const result = await runtimeFiles.listDirectory(dir);
      return collect((result?.entries ?? []) as never);
    } catch {
      return [];
    }
  }
  try {
    const response = await runtimeFetch(`${getBaseUrl()}/fs/list?path=${encodeURIComponent(dir)}`, { cache: 'no-store' });
    if (!response.ok) return [];
    const data = await response.json().catch(() => null) as { entries?: unknown } | null;
    return collect(Array.isArray(data?.entries) ? (data!.entries as never) : []);
  } catch {
    return [];
  }
};

// Returns a file's last-modified time in ms, or null if unavailable. Used to detect that a plan
// file changed on disk (e.g. the planner revised it) while it's open in the annotator.
export const statFileMtime = async (path: string): Promise<number | null> => {
  const runtimeFiles = getRuntimeFilesAPI();
  if (runtimeFiles?.statFile) {
    try {
      const result = await runtimeFiles.statFile(path);
      return typeof result?.mtimeMs === 'number' ? result.mtimeMs : null;
    } catch {
      return null;
    }
  }
  try {
    const response = await runtimeFetch(`${getBaseUrl()}/fs/stat?path=${encodeURIComponent(path)}`, { cache: 'no-store' });
    if (!response.ok) return null;
    const data = await response.json().catch(() => null) as { mtimeMs?: unknown } | null;
    return typeof data?.mtimeMs === 'number' ? data.mtimeMs : null;
  } catch {
    return null;
  }
};
...
export async function getProjectContextData(project: ProjectRef): Promise<OpenChamberProjectContextData> {
  const config = await readOpenChamberConfig(project);
  const plans = await reconcileProjectPlanFiles(project, sanitizeProjectPlanFileLinks(config?.projectPlanFiles));
...
// Reconciles the stored projectPlanFiles list against what's actually on disk in <project>/plans:
//  - keeps registered entries whose file still exists,
//  - adds any .md file present on disk that isn't registered (agent-written plans, dropped-in
//    files) — matched by basename so a differently-rooted stored path still counts as present,
//  - drops entries whose file is gone (deleted/renamed).
// Persists the result back to config only when it actually changed, so the Plans panel shows the
// real plans on every device (the list is server-side) without a manual refresh. Falls back to the
// passed-in list if the plans dir can't be resolved/listed, so a transient FS hiccup never blanks
// an existing list.
async function reconcileProjectPlanFiles(
  project: ProjectRef,
  registered: OpenChamberProjectPlanFileLink[]
): Promise<OpenChamberProjectPlanFileLink[]> {
  // Plans live in TWO places and both must be reconciled:
  //  - the hidden per-project storage dir (~/.config/openchamber/projects/<id>/plans) where the UI
  //    "New plan" action writes, and
  //  - the real repo dir <project>/plans where the planner/plan-writer agents and the Improvement
  //    "approve" flow write (the convention documented in the project's own CLAUDE.md).
  // Before this, the panel only knew about the first, so agent-authored plans never showed up.
  const storageDir = await getProjectPlansDirectory(project);
  const repoDir = project?.path ? joinPath(project.path, 'plans') : null;
  const dirs = [storageDir, repoDir].filter((d): d is string => !!d);
  if (dirs.length === 0) return registered;

  const listed = await Promise.all(dirs.map((d) => listMarkdownFilesIn(d)));
  // De-dupe by basename across both dirs (a file with the same name in both is one logical plan;
  // the storage-dir copy wins since that's what UI actions manage).
  const onDiskMap = new Map<string, { path: string; name: string; modifiedTime?: number }>();
  for (const entries of listed) {
    for (const f of entries) {
      const key = f.name.toLowerCase();
      if (!onDiskMap.has(key)) onDiskMap.set(key, f);
    }
  }
  const onDisk = [...onDiskMap.values()];
  // A dir that lists as empty could be genuinely empty OR an unreadable/absent dir — only trust an
  // empty result to prune when we can tell the difference. listMarkdownFilesIn returns [] for both,
  // so to stay safe we only prune registered entries when there's at least one file on disk (i.e.
  // the dir is definitely readable). With zero on disk we keep the registered list untouched.
  const diskByBase = new Map(onDisk.map((f) => [f.name.toLowerCase(), f]));
  const registeredBases = new Set(registered.map((r) => basename(r.path).toLowerCase()));

  let next = registered;
  let changed = false;

  if (onDisk.length > 0) {
    const kept = registered.filter((r) => diskByBase.has(basename(r.path).toLowerCase()));
    if (kept.length !== registered.length) changed = true;
    const added = onDisk
      .filter((f) => !registeredBases.has(f.name.toLowerCase()))
      .map((f) => ({ id: createProjectPlanId(), path: f.path, createdAt: f.modifiedTime ?? Date.now() }));
    if (added.length > 0) changed = true;
    // Newest first, matching how UI-created plans are prepended.
    next = [...added, ...kept].sort((a, b) => (b.createdAt ?? 0) - (a.createdAt ?? 0));
  }

  if (changed) {
    await saveProjectPlanFiles(project, next);
  }
  return sanitizeProjectPlanFileLinks(next);
}
