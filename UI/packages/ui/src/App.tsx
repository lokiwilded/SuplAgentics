...
function App({ apis }: AppProps) {
  ...
  // The chat column's width caps (.chat-column etc.) are a fixed rem value regardless of how much
  // space is actually free — on a narrower window/screen where that cap still leaves a large
  // unused gap once both side panels are closed, it reads as "the chat doesn't use the space."
  // With nothing else claiming that width, there's no readability reason left to hold the cap
  // down, so let it use nearly the full viewport in that specific case.
  React.useEffect(() => {
    const bothSidebarsClosed = !isSidebarOpen && !isRightSidebarOpen;
    document.documentElement.classList.toggle('sidebars-collapsed', bothSidebarsClosed);
    return () => {
      document.documentElement.classList.remove('sidebars-collapsed');
    };
