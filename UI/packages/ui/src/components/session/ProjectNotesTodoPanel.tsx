...
import PlanAnnotator from '../suplagentics/plan-annotator/PlanAnnotator';
import type { Annotation } from '../suplagentics/plan-annotator/types';
...
interface ProjectNotesTodoPanelProps {
  ...
  className?: string;
  // Called once plan feedback actually sends successfully (not on failure). Desktop leaves this
  // unset — its Context tab is a persistent sidebar, nothing to "go back" from. Mobile uses it to
  // close the whole Context surface and land back on chat, instead of leaving the user on the
  // Plans list they came from.
  onPlanFeedbackSent?: () => void;
}
...
// A build stamps `**Completed:** \`<ISO timestamp>\`` near the top of a plan when it finishes, so
// the panel can badge finished plans without any extra state store. Read from the raw file (which
// toPlanListItem already loads for the title) — tolerant of backticks and surrounding whitespace.
const parseCompletedAt = (raw: string | undefined | null): string | null => {
  if (typeof raw !== 'string') return null;
  const m = raw.match(/^\s*\*\*Completed:\*\*\s*`?([^`\n]+?)`?\s*$/m);
...
const toPlanListItem = async (
  ...
  return {
    ...
    completedAt: parseCompletedAt(file?.raw),
  };
};
...
export const ProjectNotesTodoPanel: React.FC<ProjectNotesTodoPanelProps> = ({
  ...
  // SuplAgentics fork port (plans/openchamber-fork-port.md) — reviewing/annotating a plan is a
  // separate, ported feature layered on top of this panel's existing native plan list; it doesn't
  // touch handleOpenPlan's own context-panel-tab viewer above.
  const [annotatingPlan, setAnnotatingPlan] = React.useState<ProjectPlanListItem | null>(null);
  const [annotatingPlanContent, setAnnotatingPlanContent] = React.useState<string>('');

  const loadAnnotatorContent = React.useCallback(async (planPath: string): Promise<string> => {
    // Read the LIVE plan via the suplagentics route (works for plans in ANY project/drive, unlike
    // /api/fs/read which is locked to the active workspace) + cache-bust, so a planner revision made
    // during "Send for Review" is reflected on reload instead of showing the stale version.
    let raw = '';
    try {
      const response = await runtimeFetch(`/api/suplagentics/plans/content?path=${encodeURIComponent(planPath)}&_t=${Date.now()}`, { cache: 'no-store' });
      if (response.ok) raw = ((await response.json()) as { content?: string })?.content ?? '';
    } catch {
      // fall back to the generic reader below
    }
    if (!raw) {
      const file = await readProjectPlanFile(planPath);
      raw = file?.body ?? file?.raw ?? '';
    }
    // Strip the origin-session marker the build flow stamps at the bottom of the file — it's
    // bookkeeping, not plan content, and shouldn't show up in the review.
    return raw.replace(/\n*<!--\s*suplagentics-session:\s*\S+\s*-->\s*$/, '');
  }, []);

  const handleOpenAnnotator = React.useCallback(
    async (plan: ProjectPlanListItem) => {
      setAnnotatingPlanContent(await loadAnnotatorContent(plan.path));
      setAnnotatingPlan(plan);
    },
    [loadAnnotatorContent]
  );

  // Re-read the plan from disk — used by the annotator's "This plan has been updated" refresh, so
  // a revision made by the planner while the review is open can be pulled in on demand (no
  // auto-reload, no watcher — the user decides when to see the new content).
  const handleReloadAnnotator = React.useCallback(async (): Promise<void> => {
    if (!annotatingPlan) return;
    setAnnotatingPlanContent(await loadAnnotatorContent(annotatingPlan.path));
  }, [annotatingPlan, loadAnnotatorContent]);

  const handleAnnotatorApprove = React.useCallback(async () => {
    // Approve = build. Kick off the build in the session this plan came from (continued if that
    // chat is still alive, otherwise a fresh commander session), then drop the user into that
    // conversation so they can carry the build the rest of the way themselves.
    if (!annotatingPlan) return;
    try {
      const response = await runtimeFetch('/api/suplagentics/plans/build', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ path: annotatingPlan.path, directory: projectRef?.path }),
      });
      const body = await response.json().catch(() => null);
      if (!response.ok || !body?.ok) {
        toast.error(body?.error || t('rightSidebar.contextNotesTodo.toast.planBuildFailed'));
        return;
      }
      setAnnotatingPlan(null);
      if (body.sessionId) {
        setCurrentSession(body.sessionId, projectRef?.path);
      }
      routeToChat();
      toast.success(
        body.continued
          ? t('rightSidebar.contextNotesTodo.toast.planBuildContinued')
          : t('rightSidebar.contextNotesTodo.toast.planBuildStartedNew')
      );
      onPlanFeedbackSent?.();
    } catch {
      toast.error(t('rightSidebar.contextNotesTodo.toast.planBuildFailed'));
    }
  }, [annotatingPlan, projectRef, routeToChat, setCurrentSession, t, onPlanFeedbackSent]);

  const handleAnnotatorDeny = React.useCallback(
    async (annotations: Annotation[]): Promise<boolean> => {
      if (!annotatingPlan) return false;
      try {
        const response = await runtimeFetch('/api/suplagentics/plans/feedback', {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({
            path: annotatingPlan.path,
            annotations,
            directory: projectRef?.path,
            // Best-effort: whichever session is currently open for this project — the most
            // direct reading of "send it back to the chat that's making it." The backend falls
            // back to SuplAgentics's own tracked-session lookup if this isn't available.
            sessionId: currentSessionId ?? undefined,
          }),
        });
        if (!response.ok) {
          const body = await response.json().catch(() => null);
          toast.error(body?.error || t('rightSidebar.contextNotesTodo.toast.planFeedbackFailed'));
          return false;
        }
        const body = await response.json().catch(() => null);
        // No session had ever written this plan file (approved straight from a suggestion,
        // dropped in manually, imported from elsewhere) — the backend started a fresh review
        // session for it rather than failing, worth calling out since it's a different outcome
        // than "your existing conversation got the feedback."
        toast.success(
          body?.startedNewSession
            ? t('rightSidebar.contextNotesTodo.toast.planFeedbackStartedNewSession')
            : t('rightSidebar.contextNotesTodo.toast.planFeedbackSent')
        );
        // Only close on genuine success — a failed send keeps the annotator open with the
        // annotations intact so the user can retry instead of silently losing their review.
        setAnnotatingPlan(null);
        onPlanFeedbackSent?.();
        return true;
      } catch {
        toast.error(t('rightSidebar.contextNotesTodo.toast.planFeedbackFailed'));
        return false;
      }
    },
    [annotatingPlan, currentSessionId, projectRef, t, onPlanFeedbackSent]
  );
  ...
  return (
    <div className={cn('w-full min-w-0 space-y-3 p-3', className)}>
                ...
                <li key={plan.id} className="flex items-center gap-1.5 px-2.5 py-1.5">
                  ...
                  >
                    <span className="flex min-w-0 items-center gap-1.5">
                      ...
                      {plan.completedAt && (
                        <span
                          className="inline-flex flex-shrink-0 items-center gap-0.5 rounded-full border border-emerald-500/40 bg-emerald-500/10 px-1.5 py-0.5 text-[10px] font-medium text-emerald-500"
                          title={t('rightSidebar.contextNotesTodo.plans.completedAt', { at: plan.completedAt })}
                        >
                          <Icon name="check" className="size-2.5" />
                          {t('rightSidebar.contextNotesTodo.plans.completed')}
                        </span>
                      )}
                    </span>
                  ...
                  <button
                    type="button"
                    onClick={() => void handleOpenAnnotator(plan)}
                    className="inline-flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-md text-muted-foreground hover:text-foreground hover:bg-interactive-hover/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50"
                    title={t('rightSidebar.contextNotesTodo.plans.reviewPlan')}
                    aria-label={t('rightSidebar.contextNotesTodo.plans.reviewPlanWithTitle', { title: plan.title })}
                  >
      ...
      {annotatingPlan && createPortal(
        <PlanAnnotator
          title={annotatingPlan.title}
          plan={annotatingPlanContent}
          planKey={annotatingPlan.path}
          planPath={annotatingPlan.path}
          onReload={handleReloadAnnotator}
          onApprove={handleAnnotatorApprove}
          onDeny={handleAnnotatorDeny}
          onCancel={() => setAnnotatingPlan(null)}
        />,
        document.body
      )}
    </div>
