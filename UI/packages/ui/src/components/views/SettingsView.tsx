...
export const SettingsView: React.FC<SettingsViewProps> = ({ onClose, forceMobile, isWindowed, visiblePageSlugs, initialMobileStage = 'nav' }) => {
  ...
  // deviceInfo.isMobile requires touch input (`hasTouchInput && isMobileWidth` — see device.ts,
  // deliberately so a resized desktop window with a mouse isn't misdetected as a phone for things
  // like touch-target sizing). But this settings view's own 3-column split layout (nav + a
  // fixed-264px page sidebar + content) has no such fallback — verified live, at ~900px total
  // width the desktop layout still renders all 3 columns, leaving the content column too narrow
  // to be usable even though nothing overflows the container itself anymore (that part's a
  // separate fix). Below this width, use the same single-column stage flow regardless of touch —
  // nothing in that flow is actually touch-specific, it's just a narrower-appropriate layout.
  const SETTINGS_SINGLE_COLUMN_WIDTH = 900;
  const isNarrowForSettingsLayout = deviceInfo.screenWidth < SETTINGS_SINGLE_COLUMN_WIDTH;
  const isMobile = forceMobile ?? (deviceInfo.isMobile || isNarrowForSettingsLayout);
  ...
  const getPageTitle = React.useCallback((slug: SettingsPageSlug): string => {
    switch (slug) {
      ...
      case 'indexing':
        return t('settings.page.indexing.title');
      case 'import':
        return t('settings.page.import.title');
      case 'improvement':
        return t('settings.page.improvement.title');
      case 'about':
