...
export const PlanView: React.FC<PlanViewProps> = ({ targetPath = null }) => {
  ...
  // Tracks the plan content as last read from disk, so the live-reload poll below can distinguish an
  // EXTERNAL change (e.g. "Send for Review" rewrote the file) from the user's own unsaved edits.
  ...
  // Live-reload the plan from disk while it's open. "Send for Review" (and any external edit) rewrites
  // the file, but the read effect above only re-runs when the PATH changes — so without this the panel
  // shows the stale version until a manual page refresh. Poll for external changes and adopt them, but
  // ONLY when there are no unsaved local edits (content === last-loaded), so it never clobbers typing.
  React.useEffect(() => {
    if (!resolvedPath) return;
    let stopped = false;
    const reload = async () => {
      try {
        const response = await runtimeFetch(`/api/fs/read?path=${encodeURIComponent(resolvedPath)}&optional=true`, { cache: 'no-store' });
        if (stopped || !response.ok) return;
        const fresh = await response.text();
        if (stopped) return;
        if (fresh !== lastLoadedRef.current && contentRef.current === lastLoadedRef.current) {
          lastLoadedRef.current = fresh;
          setContent(fresh);
        }
      } catch {
        // transient read error — just try again next tick
      }
    };
    const intervalId = window.setInterval(reload, 2500);
