...
import { MobileContextSurface } from './MobileContextSurface';
...
// This is a genuinely separate curated allowlist from the desktop settings nav's own pageOrder
// (SettingsView.tsx) — it's what actually gates which pages exist at all on this dedicated mobile
// app shell (Capacitor-style, distinct from a resized desktop browser window). Verified live: it
// had drifted to exclude 11 real settings pages — Agents, Plugins, Skills, Skills Catalog,
// Commands, Projects, Remote Instances, Snippets, Remote Tunnel, plus both SuplAgentics-added
// pages (Import History, Improvements) which never had a chance to be added since this list
// predates them. Kept in the same priority order as the desktop pageOrder for consistency, though
// SettingsView's own sortedFilteredPages re-sorts by pageOrder regardless of this list's order.
...
type OverflowItem = {
  key: 'files' | 'changes' | 'mcp' | 'context' | 'instances' | 'update' | 'settings';
  icon?: IconName;
      ...
      >
        ...
        {contextOpen ? (
          <MobileSurfaceShell
            ...
            ariaLabel={t('mobile.menu.context')}
            headerless
          >
            <ErrorBoundary>
              <MobileContextSurface onClose={() => setContextOpen(false)} />
            </ErrorBoundary>
          </MobileSurfaceShell>
        ) : null}
