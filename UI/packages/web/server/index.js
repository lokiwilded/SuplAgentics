...
const openCodeLifecycleRuntime = createOpenCodeLifecycleRuntime({
  ...
  onOpenCodeRestarted: () => {
    const cleared = sessionRuntime.resetAllSessionActivityToIdle();
    // If a restart actually interrupted live turns, ping the user. The native notification only
    // fires when the tab is unfocused; when it's focused the spinner already cleared above. Reuses
    // the existing openchamber:notification path — no client change needed.
    if (cleared > 0) {
      broadcastUiNotification({
        title: 'opencode restarted',
        body: `Recovered ${cleared} interrupted session${cleared === 1 ? '' : 's'} — resend your last message if it didn't finish.`,
        tag: 'opencode-restart',
      });
    }
    return cleared;
  },
