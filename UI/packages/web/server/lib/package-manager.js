...
const PACKAGE_NAME = 'suplagentics';
const PACKAGE_PATH_SEGMENTS = PACKAGE_NAME.split('/');
const NPM_REGISTRY_URL = `https://registry.npmjs.org/${PACKAGE_NAME}`;
const CHANGELOG_URL = 'https://raw.githubusercontent.com/lokiwilded/openchamber/main/CHANGELOG.md';
let cachedDetectedPm = null;

function getSpawnSyncBaseOptions() {
  return process.platform === 'win32' ? { windowsHide: true } : {};
}
// Upstream's own hosted telemetry+update-check service (api.openchamber.dev) has no idea this is
// a different fork under a different package name — calling it by default would both send real
// device/usage telemetry to a third party this fork doesn't control, and report UPSTREAM's own
// version as if it were this package's, since the API has no way to know otherwise. Left unset
// by default; the plain npm-registry-based check in getLatestVersion() below (already correct —
// it uses this fork's own PACKAGE_NAME) is the real fallback path and works standalone.
const UPDATE_CHECK_URL = process.env.OPENCHAMBER_UPDATE_API_URL || null;
...
export async function checkForUpdates(options = {}) {
  ...
  if (currentVersion !== 'unknown' && UPDATE_CHECK_URL) {
    ...
    if (remote) {
      ...
      return {
        ...remote,
        packageManager: pm,
        updateCommand: 'suplagentics update',
      };
    }
  }
