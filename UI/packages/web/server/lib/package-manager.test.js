...
// The hosted update-check API is opt-in (disabled by default — see package-manager.js's own
// comment on why: it's upstream's own service, wrong to hit by default from a differently-named
// fork). These tests exercise that code path directly, so they opt in explicitly, at module load
// time (UPDATE_CHECK_URL is computed once when package-manager.js itself loads, same convention
// as this file's other env-driven constants) — before importing the module under test.
