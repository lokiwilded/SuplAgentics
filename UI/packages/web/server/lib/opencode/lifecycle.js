...
// The claude-import-summarizer agent's own bash instructions need to know where the real,
// installed suplagentics-mcp-server package lives on THIS machine — it can't hardcode a path
// (verified live this session: an earlier version hardcoded the original author's own dev
// checkout path and broke outright on any other machine). Resolved once, here, since this is the
// one place that already builds the managed opencode process's environment — any bash tool call
// that process's agents make inherits this same env, no separate propagation needed.
let cachedSuplagenticsMcpServerDir;
function resolveSuplagenticsMcpServerDir() {
  if (cachedSuplagenticsMcpServerDir !== undefined) return cachedSuplagenticsMcpServerDir;
  try {
    const require = createRequire(import.meta.url);
    cachedSuplagenticsMcpServerDir = path.dirname(require.resolve('suplagentics-mcp-server/package.json'));
  } catch {
    cachedSuplagenticsMcpServerDir = null;
  }
  return cachedSuplagenticsMcpServerDir;
}
...
// 10s (not 5s): a healthy server that is actively streaming an LLM turn legitimately can't
// service a lightweight /global/health GET within 5s, and a false "unhealthy" reading is what
// used to escalate into a mid-turn forced restart. Override via env if needed.
const HEALTH_CHECK_TIMEOUT_MS = parsePositiveInt(process.env.OPENCHAMBER_OPENCODE_HEALTH_TIMEOUT_MS, 10000);
...
export const createOpenCodeLifecycleRuntime = (deps) => {
    ...
    try {
      const serverInstance = await createManagedOpenCodeServerProcess({
        ...
        env: {
          ...
          ...(resolveSuplagenticsMcpServerDir()
            ? { SUPLAGENTICS_MCP_SERVER_DIR: resolveSuplagenticsMcpServerDir() }
            : {}),
        },
      });
  ...
  const restartOpenCode = async () => {
    ...
    state.currentRestartPromise = (async () => {
      ...
      // A turn that was in-flight when the old process died never gets its 'idle' status event from
      // the fresh opencode, so any session left 'busy' would spin forever in the dashboard. Reset +
      // broadcast so those spinners clear the moment the new server is up.
      try {
        const cleared = onOpenCodeRestarted();
        if (cleared) console.log(`[lifecycle] restart: cleared ${cleared} stale busy session(s)`);
      } catch (error) {
        console.warn('[lifecycle] onOpenCodeRestarted failed:', error?.message ?? error);
      }
    })();
   ...
   */
  // 10 min (not 2): this is the last-resort backstop for a session wedged in "busy" state — it
  // must be long enough that a legitimately long multi-agent turn on slow cloud models is never
  // mistaken for a hang and killed mid-flight. A genuinely dead process is still restarted
  // immediately (that path checks isManagedOpenCodeProcessAlive, not this timer). Override via env.
  const STALE_BUSY_GRACE_MS = parsePositiveInt(process.env.OPENCHAMBER_OPENCODE_STALE_BUSY_GRACE_MS, 10 * 60 * 1000);
  ...
  // Best-effort breadcrumb so a health-monitor-triggered restart is diagnosable after the fact
  // (the dashboard's console is stdio:'inherit' and not captured anywhere). Never throws.
  const RESTART_LOG = path.join(os.homedir(), '.local', 'share', 'opencode', 'log', 'openchamber-restarts.log');
  const recordManagedRestart = (reason) => {
    try {
      const activeCount = getActiveSessionCount();
      appendFileSync(
        RESTART_LOG,
        `${new Date().toISOString()} reason=${reason} activeSessions=${activeCount} consecutiveFailures=${consecutiveHealthFailures}\n`
      );
    } catch {
      // diagnostics only — a failed write must never affect restart behavior
    }
  };
    ...
    healthCheckCyclePromise = (async () => {
      ...
      if (!healthy) {
        if (!isManagedOpenCodeProcessAlive()) {
          ...
          recordManagedRestart(`${source}:process-exited`);
          consecutiveHealthFailures = 0;
        ...
        recordManagedRestart(`${source}:health-threshold`);
        consecutiveHealthFailures = 0;
        lastHealthProbeResult = null;
        await restartOpenCode();
      } else {
