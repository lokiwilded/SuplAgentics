...
export const createSessionRuntime = ({ writeSseEvent, getNotificationClients, broadcastEvent }) => {
  ...
  // Force every tracked session to idle AND tell the clients. Called when opencode restarts:
  // a turn killed mid-flight never gets its 'idle' session.status event from opencode, so without
  // this the dashboard shows a spinner forever. Reuses the same events the UI already handles for
  // normal idle transitions, so no client-side change is needed.
  const resetAllSessionActivityToIdle = () => {
    ...
    const affected = [];
    for (const [sessionId, data] of sessionActivityPhases) {
      if (data.phase !== 'idle') affected.push(sessionId);
      sessionActivityPhases.set(sessionId, { phase: 'idle', updatedAt: now });
    }
    for (const sessionId of affected) {
      if (typeof broadcastEvent === 'function') {
        broadcastEvent({ type: 'openchamber:session-activity', properties: { sessionId, phase: 'idle' } });
      }
      // Also flip the durable state (drives a late-connecting client's initial snapshot).
      const existing = sessionStates.get(sessionId);
      if (existing && existing.status !== 'idle') {
        updateSessionState(sessionId, 'idle', `reset-${now}`);
      }
    }
