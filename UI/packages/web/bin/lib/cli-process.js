...
function isOpenchamberCmdline(cmdline) {
  ...
  // A source checkout's path still contains "openchamber" (the repo directory name), but a real
  // globally-installed package lives under node_modules/suplagentics/... (the published package
  // name, since this fork renamed it — see package.json) — neither substring alone covers both
  // the dev-checkout and the installed-package case, so both are checked. Matching a real path
  // segment (not a generic "cli.js") keeps a recycled stranger such as "npm-cli.js" or
  // "agentmemory" from being mistaken for us.
  const lower = cmdline.toLowerCase();
  return lower.includes('openchamber') || lower.includes('suplagentics');
}
